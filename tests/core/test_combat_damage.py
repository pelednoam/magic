...
def test_an_attacker_short_of_lethal_for_two_kills_only_the_first() -> None:
    ...
    assert outcome.blockers_lost == ("A",)
...
def test_two_blockers_can_gang_up() -> None:
    ...
    assert outcome.attackers_lost == ("Giant",)
    assert outcome.blockers_lost == ("A", "B"), "lethal to each in turn (CR 510.1a)"
...
def test_a_lifelink_block_can_make_a_lethal_attack_survivable() -> None:
    ...
    assert outcome.life_swing_against(1) == 2, "one life, plus the one gained"
